...
onmessage = function (e) {
    if (e.data && e.data.task == 'compile') {

        loadStyleSheet(e.data.sheet, function (e, root, _, sheet, env) {
            if (env.local) {
                log("loading " + sheet.href + " from cache.");
            } else {
                log("parsed " + sheet.href + " successfully.");
...
function loadStyleSheet(sheet, callback, reload, remaining) {
    ...
    var hrefParts = extractUrlParts(sheet.href, location.href);
...
function xhr(url, type, callback, errback) {
    ...
    function t3Preprocess (xhr, url) {
        ...
        /*
        if(
            T3Theme &&                                               //must be in thememagic mode
        ...
        */
        ...
        var vars = false,//T3Theme ? T3Theme.vars : false,
            variables = '';
