...
class JFormFieldJaDepend extends JFormField
{
    ...
    protected function profile()
    {
        ...
        if (file_exists($xmlparams)) {
            /* For General Form */
            $jaform = JForm::getInstance('jform', $xmlparams, array('control' => 'jaform'));
		...
		if($level==1){
			$html = '
				<h4 rel="'.$level.'" class="block-head block-head-'.$class_name.' open '.$class.' " '.$group.' id="'.$regionID.'">
					...
					<a class="toggle-btn open" title="'.JText::_('Expand all').'" onclick="JADepend.showseg(\''.$regionID.'\', \'level'.$level.'\'); return false;">'.JText::_('Expand all').'</a>
					<a class="toggle-btn close" title="'.JText::_('Collapse all').'" onclick="JADepend.showseg(\''.$regionID.'\', \'level'.$level.'\'); return false;">'.JText::_('Collapse all').'</a>
		    	</h4>';
		} else {
			$html = '
				<h4 rel="'.$level.'" class="block-head block-head-'.$class_name.' open '.$class.' " '.$group.' id="'.$regionID.'">
					<span class="block-setting" >'.$_title.$_url.'</span> 
					<span class="icon-help editlinktip hasTip" title="'.htmlentities($_description).'">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
					<a class="toggle-btn" title="'.JText::_('Click here to expand or collapse').'" onclick="JADepend.segment(\''.$regionID.'\', \'level'.$level.'\'); return false;">open</a>
		    	</h4>';
		} 
	...
	function ajax(){
		...
		foreach ($this->element->children() as $option):
			...
			$fcalls[] = 'JADepend.addajax(\'' . $this->getName($option['for']) . '\', ' . json_encode($fparams) . ');';
		endforeach;
		?>
		<script type="text/javascript">
			//<![CDATA[
