...
!function($){
	...
	var JADepend = window.JADepend = window.JADepend || { 	
		...
		addajax: function(name, info){
			...
			info = $.extend({
				url: info.site == 'admin' ? JADepend.adminurl : JADepend.rooturl,
				func: ''
			}, info);
	...
	var JAFileConfig = window.JAFileConfig = window.JAFileConfig || {
		...
		deleteProfile: function(){
			if(confirm(JAFileConfig.langs.confirmDelete)){			
				this.submitForm(JAFileConfig.mod_url + '?jaction=delete&profile=' + this.vars.active + '&template='+ JAFileConfig.template, {}, 'profile');
			}		
		},
		...
		duplicateProfile: function (){
			...
			if(nname){
				...
				this.submitForm(JAFileConfig.mod_url + '?jaction=duplicate&profile=' + nname + '&from=' + this.vars.active + '&template=' + JAFileConfig.template, {}, 'profile');
			}
		},
