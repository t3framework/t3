	...
	*/
	...
	function compile($env) {
		...
		if( $evald_path && $env->isPathRelative($evald_path) ){
			...
			$temp = T3Path::cleanPath($evald->currentFileInfo['rootpath'] . $evald_path);

			//top priority
			if(file_exists($temp)){
				$full_path = Less_Environment::NormPath($temp);
				$uri = Less_Environment::NormPath(dirname($evald->currentFileInfo['uri_root'].$evald_path));
			} else {

				//search for importdirs
